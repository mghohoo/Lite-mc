...
public class NewJREUtil {
    ...
    public static boolean installNewJreIfNeeded(Activity activity, JMinecraftVersionList.Version versionInfo) {
        ...
        if (versionInfo.javaVersion == null || "jre-legacy".equalsIgnoreCase(versionInfo.javaVersion.component)) {
            String java8 = MultiRTUtils.getExactJreName(8);
            if (java8 == null) {
                tryDownloadRuntime(activity, 8);
                java8 = MultiRTUtils.getExactJreName(8);
            }
            if (java8 == null) throw new IllegalStateException("Java 8 runtime is unavailable");
            LauncherProfiles.load();
            LauncherProfiles.getCurrentProfile().javaDir = Tools.LAUNCHERPROFILES_RTPREFIX + java8;
