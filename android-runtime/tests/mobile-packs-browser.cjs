...
if(process.type==='renderer') {
  ...
  window.LiteNative={request(id,action,json){const args=JSON.parse(json);window.testRequests.push({action,args});setTimeout(()=>{
    ...
    if(action==='mods.packVersions')data={items:[{id:'fabric-v',name:'稳定版 Fabric',gameVersions:['1.20.1'],loaders:['fabric']},{id:'forge-v',name:'Forge 特别版',gameVersions:['1.21.1'],loaders:['forge']}]};
  ...
  app.whenReady().then(async()=>{
    ...
    try {
      ...
      await evaluate("$('mod-results').querySelector('button').click()");await waitFor("!busy && $('pack-version').options.length===2");
      ...
      assert.equal(await evaluate("$('pack-install').disabled"),true,'unsupported Forge install must be disabled');
      assert.equal(await evaluate("$('pack-download').disabled"),false,'Forge archive download stays available');
      ...
      assert.equal(download.versionId,'forge-v');assert.equal(download.projectId,'abc');assert.equal(download.instanceId,undefined);
      ...
      win.setSize(1180,850);await evaluate("$('mod-results').querySelector('button').click()");await waitFor("!busy && $('pack-version').options.length===2");
      ...
      console.log('PASS no-instance search, popular search, version page, pinned download, unsupported loader, cancel/error/retry, XSS safety, phone/tablet layout');app.exit(0);
